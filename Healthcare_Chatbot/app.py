    ...
    with open(r'D:\HEALTHCARE_CHATBOT_PROJECT\Disease-Symptom-Prediction-Chatbot\Medical_dataset\Indian_doctors_dataset.csv', mode='r') as csv_file:
        ...
        for row in csv_reader:
            ...
            hospitalDoctorDictionary[key].append({
                ...
                # "awards": {
                #     "Padma_Vibhushan": row[5],
                #     "Padma_Bhushan": row[6],
                #     "Padma_Shri": row[7],
                #     "Dhanvantari_Award": row[8],
                #     "BC_Roy_National_Award": row[9],
                #     "Other_Awards": row[10]
                # }
            })
...
def get_bot_response():
    ...
    if 'step' not in session:
        if s.upper() in ["OK", "OKAY", "SURE", "YES", "ALRIGHT", "YES, PLEASE", "GO AHEAD", "PROCEED", "LET'S GO", "I'M READY", "AFFIRMATIVE"]:
            ...
            # Prompt the user to type "OK" if not provided
    ...
    if session['step'] == "FS":
        ...
        print("First Symptom Information (temp):", temp)
    ...
    if session['step'] == "RS1":
        ...
        print("Final First Symptom info :" ,temp)
    ...
    if session['step'] == "SS":
        ...
        print("Second Symptom info : ",temp)
    ...
    if session['step'] == "RS2":
        ...
        temp[2] = psym2
        session['SSY'] = temp
        print("Final second Symptom info :" ,temp)
        session['step'] = "semantic"
    ...
    if session['step'] == "sim1=0":  # semantic no => suggestion
        ...
        if sim1 == 0:
            if "suggested" in session:
                ...
                if s.lower() in ["yes", "yup", "s", "yeah", "yep", "sure","Ha"]:
                    psym1 = sugg[0]
                    sim1 = 1
                    temp = session["FSY"]
                    temp[1] = sim1
    ...
    if session['step'] == 'PD':
        ...
        sim1 = temp[1]
        psym1 = temp[2]
        print("review for s1: ",temp)
        temp = session["SSY"]
        sim2 = temp[1]
        psym2 = temp[2]
        print("review for s2: ",temp)
        print("hey2")
        if "all" not in session:
            session["asked"] = []
            session["all"] = [col_dict[psym1], col_dict[psym2]]
            print("session all for sym 1 and 2 :",session["all"])
        session["diseases"] = possible_diseases(session["all"])
        print("session diseses :" ,session["diseases"])
        all_sym = session["all"]
        diseases = session["diseases"]
        session["stored_diseases"] = diseases.copy()
        print("Stored diseases for later use:", session["stored_diseases"])
    ...
    if session['step'] == "DIS":
        if "symv" in session:
            if len(s) > 0 and len(session["symv"]) > 0:
                ...
                if s.lower() in ["yes", "yup", "s", "yeah", "yep", "sure","Ha"]:
                    ...
                    session["all"] = all_sym
                    print("new possible diseases:", possible_diseases(session["all"]))
        ...
        if len(session["symv"]) > 0:
            ...
            if len(diseases) > 0:
                ...
                print(f"Processed disease {processed_disease}, remaining diseases: {session['diseases']}")
    ...
    if session['step'] == "for_dis":
        diseases = session["diseases"]
        print("^^^^^^^^^^^^^^^^^^^^^")
        print("session[diseases]", diseases)
        print("length of diseases", len(diseases))
        ...
        if len(diseases) == 0:
            ...
            session['step'] = "DIS"
            session["symv"] = symVONdisease(df_tr, session["dis"])
            print("length of session[symv]", len(session["symv"]))

            return get_bot_response()

    if session['step'] == "PREDICT":
        result = model.predict(OHV(session["all"], all_symp_col))
        # result = model.predict(OHV(session["all"], all_symp_col).values.reshape(-1, 1))  # Convert DataFrame to NumPy array
        ...
        if session['step'] == "END":
            if result is not None:
                ...
                if not session['stored_diseases'] and not session['PD']:
                    ...
                    print("***********empty lists so result is result :) ********* ")
    ...
    if session['step'] == "Description":
        ...
        if s.upper() == "D":
                if session["disease"] in description_list.keys():
                    ...
                    # y["Description"] = "No description available."
                    # # write_json(y)
    ...
    if session['step'] == "Severity":
        ...
        if match:
            ...
            session['step'] = "Severity"  # Corrected assignment
            return "Could you please enter the number of days in numbers? For example, '2 days'."
    print(session['severity_flag'])


    if session['step']=="Precautions":
        if s.lower() in ["yes", "yup", "s", "yeah", "yep", "sure","Ha","intrested"]:
    ...
    if session['step'] == "DietSuggestion":
        if s.lower() in ["yes", "yup", "s", "yeah", "yep", "sure","Ha","intrested"]:
            # msg = f"Here are the diet suggestions for {session['disease']}:<br>"
    ...
    if session['step'] == "MedicationSuggestion":
        if s.lower() in ["yes", "yup", "s", "yeah", "yep", "sure","Ha","give","show","provide"]:
            ...
            # Assuming medicationDictionary contains the medication suggestions
    ...
    if session['step'] == "ExerciseSuggestion":
        if s.lower() in ["yes", "yup", "s", "yeah", "yep", "sure","Ha","give","show"]:
            print("**********flag*******",session.get('severity_flag'))
    ...
    if session['step'] == "ConsultDoctor":
        if s.lower() in ["yes", "yup", "s", "yeah", "yep", "sure","Ha"]:
            # Assuming specialistDictionary contains the disease-to-specialist mapping
    ...
    #     if s.lower() in ["yes", "yup", "s", "yeah", "yep", "sure","Ha"]:
    #         msg = "Please type the doctor's name or the number beside their name for more details👨‍⚕️:"
    ...
    #         msg += "Do you need info about any other doctor?👨‍⚕️(yes/no)"
    ...
    if session['step'] == "DoctorDetailsInput":
        ...
        try:
            ...
            msg += f"Hospital: {doctor['hospital']}<br>"
            msg += f"Experience: {doctor['experience']} years<br>"
            # if 'awards' in doctor and doctor['awards']:
            #     msg += f"Awards: {', '.join(doctor['awards'])}<br>"
            # else:
            #     msg += "Awards: None listed.<br>"
            
            # Ask if the user needs info about any other doctor
    ...
    #     else:
    #         session['step'] = "BYE"
    #         return "⚠️ Please respond with 'yes' or 'no'."
